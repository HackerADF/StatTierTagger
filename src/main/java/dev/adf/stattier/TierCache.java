...
public class TierCache {
    ...
    private static void fetchLeaderboard() {
        String endpoint = TierTagger.getManager().getConfig().getApiUrl() + "/leaderboard";
            ...
            TierTagger.getClient().sendAsync(request, HttpResponse.BodyHandlers.ofString())
                    .thenAccept(response -> {
                        ...
                        for (JsonElement element : array) {
                            ...
                            if (obj.has("tiers") && !obj.get("tiers").isJsonNull() && obj.get("tiers").isJsonObject()) {
                                ...
                                for (Map.Entry<String, JsonElement> entry : tiers.entrySet()) {
                                    String modeId = entry.getKey().toLowerCase(Locale.ROOT);
                                    String tierString = entry.getValue().getAsString();
                                    PlayerInfo.Ranking ranking = parseTierString(tierString);
                                    if (ranking != null) {
                                        rankings.put(modeId, ranking);
                                        modeSet.add(modeId + ":" + entry.getKey());
                                    }
                                }
                            }
                        ...
                        for (Map.Entry<String, PlayerInfo> e : PLAYERS.entrySet()) {
                            if (!e.getValue().rankings().isEmpty()) {
                                ranked.add(e);
                            }
                        }
                        ranked.sort(Comparator.comparingInt((Map.Entry<String, PlayerInfo> e) -> e.getValue().getTotalPoints()).reversed());
                        for (int i = 0; i < ranked.size(); i++) {
                            RANKS.put(ranked.get(i).getKey(), i + 1);
                        }

                        TierTagger.getLogger().info("Loaded {} players and {} game modes from leaderboard", PLAYERS.size(), GAMEMODES.size());
                    }).get();
        } catch (Exception e) {
            TierTagger.getLogger().error("Failed to load leaderboard!", e);
        }
    }
    ...
    private static PlayerInfo.Ranking parseTierString(String tierString) {
        try {
            ...
            return new PlayerInfo.Ranking(tier, pos, null, null, 0L, false);
        } catch (Exception e) {
    ...
    public static Optional<Map<String, PlayerInfo.Ranking>> getPlayerRankings(String name) {
        Map<String, PlayerInfo.Ranking> rankings = TIERS.get(name.toLowerCase(Locale.ROOT));
        if (rankings != null && rankings.isEmpty()) return Optional.empty();
        return Optional.ofNullable(rankings);
    }

    public static Optional<PlayerInfo> getPlayerInfo(String name) {
        return Optional.ofNullable(PLAYERS.get(name.toLowerCase(Locale.ROOT)));
    }

    public static int getPlayerRank(String name) {
        return RANKS.getOrDefault(name.toLowerCase(Locale.ROOT), 0);
    }
